                ...
                <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
                    {navItems.map(({ to, label, Icon }) => (
                        <NavLink key={to} to={to} end={to === '/ai'} onClick={() => setSidebar(false)}
                            className={({ isActive }) => `px-3.5 py-2.5 flex items-center gap-3 rounded ${isActive ? 'bg-linear-to-r from-[#3C81F6] to-[#9234EA] text-white' : 'text-gray-600 hover:'}`}>
                            {({ isActive }) => (
                                <>
            ...
            <div className='w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between'>
                ...
                <div>
                    ...
                    <p className='text-xs text-gray-500 mt-1'>
                        <Show when={{plan:"premium"}} fallback="Free">Premium</Show> Plan
                    </p>
                </div>
                </div>
